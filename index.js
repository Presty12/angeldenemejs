...
// Güvenliye ekleme fonksiyonu
  if(command === "güvenli") {
    ...
    if (serendiaGuvenli.some(g => g.includes(pieceTarget.id))) {
      ...
      message.channel.send(pieceEmbed.setTitle(`Santa var lan burda`).setDescription(`${pieceTarget}, ${message.author} tarafından güvenli listeden kaldırıldı!`));
    } else {
...
piece.login(ayar.botToken).then(c => console.log(`${piece.user.tag} olarak giriş yapıldı!.`)).catch(err => console.error("Bota giriş yapılırken başarısız olundu kanka bak bi tokeni falan kontrol et."));